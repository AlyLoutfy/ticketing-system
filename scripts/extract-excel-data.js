...
function extractTicketTypes(df, departmentName) {
  ...
  for (let i = headerRowIndex + 1; i < df.length; i++) {
    ...
    const existing = ticketTypes.find((t) => t.name.toLowerCase() === ticketTypeName.toLowerCase());
...
function extractAllData() {
  ...
  for (const [filename, departmentName] of Object.entries(fileDepartmentMap)) {
    ...
    try {
      ...
      const requestSheet = sheetNames.find((name) => name.toLowerCase().includes("request") || name.toLowerCase().includes("ticket")) || sheetNames[1] || sheetNames[0];
